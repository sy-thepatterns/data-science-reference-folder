...
```python
loss = mse(pred, y) + alpha * model.weight.abs().sum()
```
...
| Property | Value |
|---|---|
| Primary API | `torch.nn.Linear plus explicit L1 penalty` |
| Fitting style | Iterative autograd optimization |
| Core solver route | Chosen optimizer |
| Statistical inference | None automatic |
| Sparse support | Weights may become small but not reliably exact-zero |
| GPU support | Yes |
...
Generic gradient optimizers do not reproduce coordinate-descent soft thresholding and may not create exact zeros; proximal updates are needed for faithful sparsity.
...
5. Tibshirani — “Regression Shrinkage and Selection via the Lasso”
6. Friedman, Hastie, and Tibshirani — coordinate descent
